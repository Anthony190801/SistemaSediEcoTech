...
@section('title', 'Editar Proyecto - SEDIECOTECH Rewards')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Modifica la información del proyecto</p>
        </div>
        <a 
    ...
    <!-- Form -->
    <form action="{{ route('admin.proyectos.update', $proyecto) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-400">Deja vacío para mantener el logo actual</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="logoPreview" class="w-full h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50 overflow-hidden">
                            @if($proyecto->url_logo)
                                <img src="{{ asset('storage/' . $proyecto->url_logo) }}" alt="{{ $proyecto->nombre }}" class="max-h-full max-w-full object-contain rounded-lg">
                            @else
                                <p class="text-gray-400 text-sm">No hay logo actual</p>
                            @endif
                        </div>
                    </div>
...
<script>
    ...
    const institucionesExistentes = @json($institucionesExistentes);
    let institucionCounter = 0;

    // Cargar instituciones existentes al cargar la página
    document.addEventListener('DOMContentLoaded', function() {
        institucionesExistentes.forEach(inst => {
            addInstitucion(inst);
        });
    });
    ...
    function previewLogo(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            @if($proyecto->url_logo)
                preview.innerHTML = `<img src="{{ asset('storage/' . $proyecto->url_logo) }}" alt="{{ $proyecto->nombre }}" class="max-h-full max-w-full object-contain rounded-lg">`;
            @else
                preview.innerHTML = '<p class="text-gray-400 text-sm">No hay logo actual</p>';
            @endif
        }
    }
</script>
